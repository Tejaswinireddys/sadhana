...
- Pose media discovery: `GET /api/poses/:slug/media` → `{ video: {hls, mp4, webm, poster, captions} | null, audio: {url, source: human|neural, cues:[{t,text}]} | null }` (`server/poseMediaManifest.ts`). Client helpers in `client/src/lib/poseMediaApi.ts` — prefer these over guessing `/videos/...` or `/voice/...` paths.
...
- CSP `script-src` in **production** is `'self'` + the hashed theme boot script (no `'unsafe-inline'`). In **development**, `script-src` is `'self' 'unsafe-inline'` only — a hash would disable inline and break Vite’s React-refresh preamble (blank SPA). `style-src` still allows `'unsafe-inline'` for React `style={}`.
