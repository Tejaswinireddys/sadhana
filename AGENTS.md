...
- Pose media discovery: `GET /api/poses/:slug/media` → `{ video: {hls, mp4, poster, playbackId, provider} | null, audio… }` (`server/poseMediaManifest.ts`). Adaptive video via **Bunny Stream** by default (`STREAM_PROVIDER`, `BUNNY_STREAM_CDN_HOSTNAME`); playback IDs in `pose_media` / `.data/pose-stream-ids.json`. Client uses **hls.js** (`preload="metadata"`, poster = pose PNG); GuidedSession preloads the next clip. Stream failure → illustration, never a blocking spinner.
