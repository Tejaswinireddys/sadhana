...
- **Marketing UX:** `/welcome` is quiz-first (primary CTA → `/start`). `/start` and `/verify` are chrome-free (no sidebar). The practice app shell stays for `/` and in-app routes.
- **Accounts:** `POST /api/auth/signup` creates an unverified user and does **not** set a session cookie. Users must hit `/verify` (email link or pasted code) via `POST /api/auth/verify-email` before login. Without `RESEND_API_KEY`, verification/reset emails are logged to the server console; non-production also returns `verifyToken` / `resetToken` in JSON for local flows. Existing Postgres users get `email_verified = true` via the schema default/ALTER.
