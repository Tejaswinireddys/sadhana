...
- Pose media discovery: `GET /api/poses/:slug/media` → `{ video: {hls, mp4, poster} | null, audio: {url, cues} | null }` (`server/poseMediaManifest.ts`). Client helpers in `client/src/lib/poseMediaApi.ts` — prefer these over guessing `/videos/...` or `/voice/...` paths.
