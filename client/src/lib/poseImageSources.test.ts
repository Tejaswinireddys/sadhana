...
describe("pose thumbnail source selection", () => {
  ...
  it("falls back to full-size assets for warm-up poses when no thumbs are ready", () => {
    for (const step of WARMUP.steps) {
      const resolved = resolvePoseImageSources(step.asanaSlug, { thumb: true, baseUrl: "/" });
      assert.equal(resolved.usesThumb, POSE_THUMBS_READY_LIST.includes(step.asanaSlug));
      if (!resolved.usesThumb) {
        assert.match(resolved.src, new RegExp(`/poses/${step.asanaSlug}\.png$`));
