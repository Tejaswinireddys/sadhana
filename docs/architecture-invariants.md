...
| Device-ID ownership | `lib/deviceId`, `server/owner` | `X-Device-Id` scoping; a valid account session overrides it with `user:<id>` |
| Account sessions | `server/auth`, `lib/auth` | Cookie name, scrypt hashes, owner switch clearing cached queries |
